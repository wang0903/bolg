...
spec:
  hosts:
  - {name: master, address: 192.168.200.235, internalAddress: 192.168.200.235, user: root, password: "62352744aa??aa"}
  - {name: node01, address: 192.168.200.236, internalAddress: 192.168.200.236, user: root, password: "62352744aa??aa"}
  - {name: node02, address: 192.168.200.237, internalAddress: 192.168.200.237, user: root, password: "62352744aa??aa"}
  roleGroups:
    etcd:
    - master
    control-plane:
...
pipeline {
    ...
    environment {
        harborUser = 'admin'
        harborPasswd = 'TG62352744aa??aa'
        harborAddress = '192.168.200.239:30002'
        harborRepo = 'tgprod'
    ...
    stages {
        stage('检出') {
            steps {
                checkout scmGit(branches: [[name: '${tag}']], extensions: [], userRemoteConfigs: [[credentialsId: '5e3afd73-6581-4e14-8bd4-f17e3f353270', url: 'http://192.168.200.232/root/tg.git']])
            }
        }
        stage('项目构建') {
            steps {
...
image:
  repository: 192.168.200.239:30002/tgtestprod/tg-server  # 镜像仓库地址，指定从哪里拉取镜像
...
image:
  # 镜像仓库地址
  repository: 192.168.200.239:30002/tgweb/tg-web
  # 镜像标签，这里可以传入变量进行替换
  tag: "latest"
...
- apiGroups: ["traefik.io"]  # Traefik API 组
  ...
  verbs: ["get", "list", "watch", "create", "update", "delete"]  # 相关操作权限
```
...
```sh
git clone http://192.168.200.232/root/tg.git
cd tg
...
git remote add origin http://192.168.200.232/root/tg.git
git add .
