...
![架构图](../../.vuepress/public/assets/images/wmremove-transformed.jpeg)
...
apiVersion: v2
name: loki-rustfs
description: Loki + rustfs offline chart
type: application
version: 0.1.0
appVersion: "1.16.0"
```
...
```yaml
image:
  loki: swr.cn-north-4.myhuaweicloud.com/ddn-k8s/docker.io/grafana/loki:3.6.4
  promtail: swr.cn-north-4.myhuaweicloud.com/ddn-k8s/docker.io/grafana/promtail:3.5.7
...
data:
  loki.yaml: |
    ...
    storage_config:
      aws:
        endpoint: http://192.168.1.233:9000
        bucketnames: loki
        region: us-east-1
        access_key_id: d0fIanAVi9xmu3GCRB
        secret_access_key: pQNCvHd0cgmsZXVDa3YEOoI8AwbrWUKlM5SFR
...
spec:
  ...
  resources:
    requests:
      storage: 5Gi        # WAL 不用太大，5~10Gi 足够
  storageClassName: nfs-client   # 改成你的 NFS StorageClass
```
## 集群安装监控
...
apiVersion: v2
name: monitoring
description: Kubernetes monitoring umbrella chart
type: application
version: 0.1.0
...
node-exporter:
  enabled: true
  image:
    repository: swr.cn-north-4.myhuaweicloud.com/ddn-k8s/quay.io/prometheus/node-exporter
    tag: v1.9.1
  service:
    type: NodePort
    port: 9100
...
cadvisor:
  enabled: true
  image:
    repository: swr.cn-north-4.myhuaweicloud.com/ddn-k8s/ghcr.io/google/cadvisor
    tag: 0.56.2
  service:
      type: NodePort
      port: 8080
      ...
      containers:
        - name: cadvisor
          ...
          args:
            ...
            - --disable_metrics=disk,diskIO,network,tcp,udp,percpu,sched,process,advtcp
            - --docker_only=true
          ports:
            - containerPort: 8080
              hostPort: 8080
...
kind: Service
metadata:
  name: {{ .Chart.Name }}  # 使用 Helm 模板来动态设置名字
  labels:
    app: {{ .Chart.Name }}  # 使用 Helm 模板来动态设置标签
spec:
  type: {{ .Values.service.type | default "NodePort" }}  # 根据 values.yaml 中的 service.type 来动态设置类型，默认为 NodePort
  ports:
    - port: 8080  # 对外暴露的端口
      targetPort: 8080  # 容器内部监听的端口
      nodePort: 30102  # 自定义 NodePort，确保集群外部访问
  selector:
    app: {{ .Chart.Name }}  # Selector 用于匹配相应的 Pod
```
### kube-state-metrics
#### values.yaml
```yaml
...
metadata:
  name: {{ .Chart.Name }}
  namespace: monitoring  # 推荐监控专用命名空间
  labels:
    app: {{ .Chart.Name }}
spec:
  selector:
