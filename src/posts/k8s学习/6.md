...
pipeline {
    ...
    stages {
        ...
        stage('拉取git代码') {
            steps {
                //checkout scmGit(branches: [[name: '${tag}']], extensions: [], userRemoteConfigs: [[url: 'http://192.168.127.142/root/mytest.git']])
                checkout scmGit(branches: [[name: '${tag}']], extensions: [], userRemoteConfigs: [[credentialsId: 'b1f83372-2a32-4a6c-bf34-52383a49a356', url: 'http://192.168.1.142/root/hzwx.git']])
            }
        }
        stage('验证 JDK') {
            steps {
        ...
        stage('通过sonarQube代码质量检测') {
            steps {
                sh '/var/jenkins_home/sonar-scanner/bin/sonar-scanner  -Dsonar.source=./  -Dsonar.projectname=${JOB_NAME}  -Dsonar.projectKey=${JOB_NAME}  -Dsonar.java.binaries=/var/jenkins_home/workspace/prod/yudao-server/target  -Dsonar.login=sqa_85799e499f7e73522113afbe231907285ef133c9'
            }
        }
        ...
        stage('将自定义镜像推送到Harbor仓库') {
            steps {
                sh '''docker login -u ${harborUser} -p ${harborPasswd} ${harborAddress}
                           docker tag yudao-server:${tag} 192.168.128.166:80/mes-server/yudao-server:${tag}
        ...
        stage('更新 DevOps 仓库中的后端镜像版本') {
            steps {
                ...
                echo "http://${GIT_USER}:${ENCODED_PASS}@192.168.127.142" > \$HOME/.git-credentials
                ...
                git clone http://192.168.127.142/root/devops.git devops-repo
                cd devops-repo
...
```yaml
# 后端服务副本数
replicaCount: 3  # 部署 3 个 Pod，提高高可用性

image:
  # Java 后端镜像仓库地址（Harbor）
  repository: 192.168.128.166:80/mes-server/yudao-server
  # 镜像 tag
  tag: v4.0.1.7
  # 镜像拉取策略：Always 表示每次启动都去拉取镜像
  pullPolicy: Always

# 环境变量设置（可在 Deployment 中使用）
env:
  - name: ENVIRONMENT   # 环境变量名称
    value: "dev"        # 环境变量值
  - name: JAVA_OPTS       # 增加 JVM 内存参数
    value: "-Xms512m -Xmx1536m -XX:+UseG1GC -Dfile.encoding=UTF-8"

service:
  type: ClusterIP       # 默认服务类型，集群内访问
  port: 48080           # Java SpringBoot 暴露的端口
...
# 存活探针，检测应用是否存活，否则重启
livenessProbe:
  path: /actuator/health    # Spring Boot 健康检查接口
  port: 48080
  initialDelaySeconds: 60   # 延迟 60 秒开始探测，避免应用未初始化成功就检查
  timeoutSeconds: 1
  periodSeconds: 30
  successThreshold: 1
  failureThreshold: 3
...
{{ include "mes-server.selectorLabels" . }}
{{- if .Chart.AppVersion }}
...
{{/*
Selector labels (must appear inside Deployment selector)
*/}}
...
*/}}
{{- define "mes-server.serviceAccountName" -}}
{{- if .Values.serviceAccount.create }}
{{- default (include "mes-server.fullname" .) .Values.serviceAccount.name }}
{{- else }}
...
spec:
  ...
  source:
    repoURL: http://192.168.127.142/root/devops.git
    targetRevision: main
  ...
  destination:
    ...
    namespace: mes-server  # 你的应用部署到的 namespace
...
![alt text](../../.vuepress/public/assets/images/k8s/123.png)

### containerd添加私有仓库(containerd默认只接收HTTPS)

> 到主节点和工作节点配置，路径为中：/etc/rancher/rke2/registries.yaml。注意：修改前一定要先备份源文件
...
mirrors:
  "docker.io":
    endpoint:
      - "https://cf-workers-docker-io-784.pages.dev"
      - "https://registry.cn-hangzhou.aliyuncs.com"
      - "https://foqmnfux.mirror.aliyuncs.com"
      - "https://docker.m.daocloud.io"

  "192.168.128.166:80":
    endpoint:
      - "http://192.168.128.166:80"
...
configs:
  ...
  "192.168.128.166:80":
    auth:
      ...
      password: "你的Harbor密码"
    tls:
      insecure_skip_verify: true   # 关键！跳过TLS校验（因你是HTTP）
```

1. 重启或者加载

   ```sh
   sudo systemctl restart rke2-server   # master 节点
   sudo systemctl restart rke2-agent    # worker 节点
   ```

2. 创建链接,让系统使用rictl下载镜像

   ```sh
   sudo ln -s /var/lib/rancher/rke2/bin/crictl /usr/local/bin/crictl
   ```

### Kubernetes安装XXL-JOB(用Helm Chart)
...
spec:
  ...
  destination:
    server: https://kubernetes.default.svc
    namespace: mes  #命名空间，最好是和后端系统放在一个空间
